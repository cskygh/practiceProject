# practiceProject$(document).ready(function() {
	toastr.options = {
...
//			$('#stock').val('');
	...
	$('#insert-btn').on('click', function() {
		if($('#proCode').val() == '' || $('#vendCode').val() == ''){
			...
			swal({
				title : '매입 수량을 입력해주시기 바랍니다.',
				text : '매입수량, 금액 또는 세액이 입력되지 않았습니다.',
				icon : 'warning',
				closeOnClickOutside: false, // alert 창을 제외하고 클릭시 창닫히지 않게(false,
											// true면 닫힘)
				closeOnEsc: false, // esc 키 안먹히게(기본 true)
	...
	$(document).on('click', '#buyInsertList td',function() {
...
//		var data = buyTable.fnGetData(this); // 데이터 테이블 클릭한 곳의 값 반환
		...
		$('#buyUpdateForm').prepend(beforeValue);
		$.ajax({
			type : 'POST',
			data : {
				seq : seq
			...
			success : function(buy) {
				...
				$('#no').val(buy.no);
				$('#hang').val(buy.hang);
				$('#vendCode').val(buy.vendCode).prop({'selected': true, 'disabled': true});
				$('#proCode').val(buy.proCode).prop('selected', true);
				settingProduct(buy.proCode); // product 나머지 값 입력
				$('#beforeProCode').val(buy.proCode);
				$('#beforePrice').val(buy.price);
				$('#price').val(buy.price);
				$('#beforeQty').val(buy.qty);
				$('#qty').val(buy.qty);
				$('#beforeTax').val(buy.tax);
				$('#tax').val(buy.tax);
				$('#beforeTotal').val(buy.total);
				$('#total').val(buy.total);
				$('#memo').val(buy.memo);
				toastr.success('매입 항목을 수정해 주시기 바랍니다.');
			},
			error : function(xhr, status, error) {
				alert('ajax error');
			}
	...
	$(document).on('click', '#update-btn', function() { // 수정 버튼, update
		...
		if($('#proCode').val() == '' || $('#vendCode').val() == ''){
			...
			return;
		} else if((qty - beforeQty + stock) < 0) {
			swal({
				title : '매출 수량을 먼저 수정해 주시기 바랍니다.',
				text : '수정할 경우 현재 재고가 -가 됩니다.',
				icon : 'warning',
				closeOnClickOutside: false, // alert 창을 제외하고 클릭시 창닫히지 않게(false,
											// true면 닫힘)
				closeOnEsc: false, // esc 키 안먹히게(기본 true)

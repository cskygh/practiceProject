...
$(document).ready(function() {
	...
	$('#vender-reset').on('click', function() { // 초기화 버튼 클릭시 input text 및
												// checkbox 초기화
		resetForm();
	...
	$('#randomCode').on('click', function() { // 코드 자동코드로 클릭 해제 시, 클릭 시에는 값
												// 초기화 및 읽기 전용으로
		if ($('#randomCode').prop('checked')) {
			var code = randomCode();
			$('#code').val(code);
	...
	$('#code').keyup(function() { // 코드에 숫자만 입력되게(keydown으로 할 경우 마지막 글자가 살아있다.
		$('#code').val($('#code').val().replace(/[^0-9]/g,''));
	});
	
	$('#bizNo').keyup(function() { // 단가에 숫자만 입력되게
		$('#bizNo').val($('#bizNo').val().replace(/[^0-9]/g,''));
	});
	
	$('#venderList #edit-btn').on('click', function(){
		var code = $(this).closest('tr').children().eq(0).text();
		location.href = 'venderEditFormList?code=' + code;
	});
	
	$('#venderList #delete-btn').on('click', function(){
		...
		swal({
			...
			if(result){
				swal({
					title : '정말로 삭제하시겠습니까?',
					text : '코드명 : ' + code + '업체명 : ' + name + '이 삭제됩니다.',
					icon : 'warning',
